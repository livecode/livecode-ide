# Introduction
You can extend the core functionality of LiveCode in the following ways:

Option|Description|Language
-------|-------|-------
Library|Create a library written in LiveCode that is loaded by the LiveCode engine at startup at engine level. Functions implemented in these libraries do not sit in the message path and exists in the global namespace just just standard LiveCode syntax.|LiveCode
Widget|Similar to a library, except it is loaded into the engine as an object which can be dragged out from the tools palette.|LiveCode
External|A low level compile library (DLL)|C,C++,Objective C, Java
...
An API for an extension is automatically added to the dictionary in the library dropdown.
...
#TO REVIEW
If the target name is shared by two references, then the above labelling also serves to disambiguate:
...
Description: This library consists of the sorting operations provided by the standard library of modular LiveCode
...
### Modular LiveCode handler example
```
/*
